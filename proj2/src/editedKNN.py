...
class EKNNErrClassifier:
    ...
    def classify(self, x, k):
        ...
        for xt in self.D:
            ...
            # we used the heappushpop function since the documentation claims it is more
            # efficient than using the two functions separately
            if len(l) >= k:
                hq.heappushpop(l, e)
            else:
                hq.heappush(l, e)
...
class EKNNTrueClassifier:
    ...
    def classify(self, x, k):
        ...
        for xt in self.D:
            # save the distance in a tuple named e with its associated class
            e = (-knn.euclidianDistance(x, xt), xt[-1])
            # we used the heappushpop function since the documentation claims
            # it is more efficient than using the two functions separately
            if len(l) >= k:
                hq.heappushpop(l, e)
            else:
                hq.heappush(l, e)
