...
def kfold(D, k, debug=False):
    ...
    random.shuffle(Vs)
    # sort the vectors in Vs by their class. Since the sort is stable, the randomization introduced by the shuffle in
    # the previous step will be preserved
...
def crossvalidation(D, c, t: tr.Classifier, k=10):
    ...
    for i in range(0, k):
        ...
        for x in tf:
            # we access the confusion matrix position defined by the predicted class and the actual class for x and we
            # increment the value in that position by 1
