...
def main():
    ...
    kmeans_results = kfxv.tenfoldcrossvalidationC(kmeans_glass, X_glass, y_glass, k=1)
    kmeans_accuracy = 1 - np.mean([ev.zero_one_loss(r[0], r[1]) for r in kmeans_results])
    print(f"kMeans kNN Accuracy: {kmeans_accuracy:.4f}")
    ...
    kmeans_results = kfxv.tenfoldcrossvalidationC(kmeans_soybean, X_soybean, y_soybean, k=1)
    kmeans_accuracy = 1 - np.mean([ev.zero_one_loss(r[0], r[1]) for r in kmeans_results])
    print(f"kMeans kNN Accuracy: {kmeans_accuracy:.4f}")
    ...
    knn_mse = 1 - np.mean([ev.mse(r[0], r[1]) for r in knn_results])
    ...
    eknn_mse = 1 - np.mean([ev.mse(r[0], r[1]) for r in eknn_results])
    ...
    kmeans_results = kfxv.tenfoldcrossvalidationR(kmeans_abalone, X_abalone, y_abalone, k=15, sig=0.25)
    kmeans_mse = 1 - np.mean([ev.mse(r[0], r[1]) for r in kmeans_results])
    ...
    kmeans_results = kfxv.tenfoldcrossvalidationR(kmeans_computer, X_computer, y_computer, k=1, sig=.25)
    kmeans_mse = 1 - np.mean([ev.mse(r[0], r[1]) for r in kmeans_results])
    print(f"kMeans kNN MSE: {kmeans_mse:.4f}")
    ...
    X_fires, y_fires = preprocess_data(hardware_filepath)
    ...
    kmeans_results = kfxv.tenfoldcrossvalidationR(kmeans_fires, X_fires, y_fires, k=13, sig=.5)
    kmeans_mse = 1 - np.mean([ev.mse(r[0], r[1]) for r in kmeans_results])
    print(f"kMeans kNN MSE: {kmeans_mse:.4f}")
