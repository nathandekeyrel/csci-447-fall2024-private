...
class PSO:
    def __init__(self, X_train, Y_train, n_nodes_per_layer, n_hidden_layers, population, inertia, cognitive_update_rate, social_update_rate, is_classifier):
    ...
    def initialize_model(self, X, Y):
        ...
        self.personal_bests = [self._performance(self.nets[i].predict(self.X_train), self.Y_train) for i in range(len(self.nets))]
    ...
    def _calculate_velocities(self):
        ...
        weight_velocities = [
            [
                ...
                self.social_update_rate * (self.personal_best_weights[self.global_best_index][j] - self.nets[i].weights[j])
        ...
        bias_velocities = [
            [
                ...
                self.social_update_rate * (self.personal_best_biases[self.global_best_index][j] - self.nets[i].biases[j])
                for j in range(self.nets[0].layers - 1)
            ]
            for i in range(len(self.nets))
        ]
